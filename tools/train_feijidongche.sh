...
# 默认双数据集；脚本仍沿用 train.sh 的组织方式，统一展开成 --dataset-dir 参数。
DATASET_DIRS="${DATASET_DIRS:-datasets/非机动车车牌/suzhou_yolo,datasets/非机动车车牌/anhui_yolo}"
...
# 基础训练参数：默认是小模型、YOLO 数据集、非机动车车牌数据集训练。
...
OUTPUT_DIR="${OUTPUT_DIR:-output/${CURRENT_DATE}_feijidongche_${MODEL_NAME}}"
