...
class OnnxOptimizer:
    ...
    def insert_layernorm_plugin(self):
        ...
        self.cleanup()
        G_LOGGER.info(f"Insert {nLayerNormPlugin} LayerNorm plugins")
        return nLayerNormPlugin

    def insert_convx_layernorm_plugin(self):
        """
            ...
            if node.op == 'Mul':
                try:
                    ...
                    for i, input_tensor in enumerate(node.inputs):
                        if type(input_tensor) == gs.ir.tensor.Constant:
                            shape = input_tensor.values.shape
                    ...
                    for _ in range(15):  # Max 15 steps back (increased for standalone LayerNorm)
                        ...
                        if prev_node.op in ['Conv', 'Add', 'Concat', 'Reshape'] and len(prev_node.inputs) > 0:
                            ...
                            # For standalone LayerNorm, the input might be from various sources
                            if len(prev_node.inputs) > 0:
                                input_tensor = current_tensor
                                break
                            break
                    ...
                    if len(add_node.outputs) > 0:
                        ...
                        # Replace in graph outputs if this is a final output
        ...
        self.cleanup()
        G_LOGGER.info(f"Insert {nConvXLayerNormPlugin} ConvNeXt LayerNorm plugins")
        return nConvXLayerNormPlugin
